...
import Logic3007 from "../game_element/body/logic_3007/Logic3007";
import utilMath from "../../frame/basic/UtilMath";
import UINodeInstEnterRS from "../../frame/ui/UINodeInstEnterRS";

const {ccclass, property} = cc._decorator;

const APP = `GameDisplayBody005`;

const MAT_PROPERTY_ENABLED_TIME = `enabledTime`;

/**
 * 旋转角锁住，脚底位置对齐，自动同步动画 + 特效时间 + 3007 专用
 */
@ccclass
export default class GameDisplayBody005 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMap = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayBody005, GamePlayViewState, number>>;
     /**
      * 获取对应的节点类型
      * @param res 
      * @returns 
    ...
    static GetNodeType (cfg: CfgGameElement) {
        ...
        if (!GameDisplayBody005._nodeTypeCacheMap.has(cfg.id)) {
            GameDisplayBody005._nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayBody005, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayBody005),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayBody005, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
                                    ...
                                    propsFilter: (com, state, props) => {
                                        ...
                                        let logic3007 = npcELe.commonBehaviour as Logic3007;
                                        com.nodePrickle.opacity = utilMath.Clamp (logic3007.hmStatusCD.cd / logic3007.args.cd, 0, 1) * 255;
                                    }
                                }
                            )
                        ],
