 ...
 */
@ccclass
export default class GameDisplayBody016 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMapForAtk = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayBody016, GamePlayViewState, number>>;
    /**
     * 获取对应的节点类型
     * @param res 
     * @returns 
    ...
    static GetNodeTypeForAtk (cfg: CfgGameElement) {
        ...
        if (!GameDisplayBody016._nodeTypeCacheMapForAtk.has(cfg.id)) {
            GameDisplayBody016._nodeTypeCacheMapForAtk.set(
                cfg.id,
                UINodeType.Pop<GameDisplayBody016, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayBody016),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayBody016, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
                                    ...
                                    propsFilter: (com, state, props) => {
                                        ...
                                        com.nodeSize.height = logic3033.args.height * 2;
                                        com.nodePos.x = -logic3033.args.height;
                                    }
                                }
                            )
                        ],
