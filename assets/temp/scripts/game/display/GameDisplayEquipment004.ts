...
const APP = `GameDisplayEquipment004`;

/**
 * 弓箭常规可视化专用 + 不考虑后座力震颤
 */
@ccclass
export default class GameDisplayEquipment004 extends UIViewComponent {
     ...
     * 已缓存了的节点
     */
    private static nodeTypeCacheMap: Map<number, UINodeType<GameDisplayEquipment004, GamePlayViewState, number>> = UtilObjPool.Pop(UtilObjPool.typeMap, APP);
    ...
    static GetNodeTypeByRes (cfg: CfgEquipmentProps) {
        if (!this.nodeTypeCacheMap.has(cfg.id)) {
            this.nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayEquipment004, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: cfg.res,
                        componentGetter: node => node.getComponent(GameDisplayEquipment004),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayEquipment004, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
                                    ...
                                    propsFilter: (com, state, props) => {
                                        ...
                                        let geei2004 = mbPlayer.playerMgrEquipment.equipInst as GEEI_2004;
                                        let angle = geei2004.atkAngle;
                                        ...
                                        com.node.x = geei2004.relEquip.machine.equip.npc.args.equipOffsetX;
                                        com.node.y = geei2004.relEquip.machine.equip.npc.args.equipOffsetY;
    
                                        com.LoadAnim(geei2004.currStatus.OnAnim());
                                        com.AnimTimeScale(state.playMachine.gameState._timeScale * gameCommon.STANDARD_ANIM_MS / geei2004.currStatus.OnMSKeep());
