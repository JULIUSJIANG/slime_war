...
/**
 * 弓箭常规可视化专用 + 不考虑后座力震颤 + 魔法书定制 + 无就绪偏移
 */
@ccclass
export default class GameDisplayEquipment006 extends UIViewComponent {
     ...
     * 已缓存了的节点
     */
    private static nodeTypeCacheMap: Map<number, UINodeType<GameDisplayEquipment006, GamePlayViewState, number>> = UtilObjPool.Pop(UtilObjPool.typeMap, APP);
    ...
    static GetNodeTypeByRes (cfg: CfgEquipmentProps) {
        if (!this.nodeTypeCacheMap.has(cfg.id)) {
            this.nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayEquipment006, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: cfg.res,
                        componentGetter: node => node.getComponent(GameDisplayEquipment006),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayEquipment006, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
