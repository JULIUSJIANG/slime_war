...
const APP = `GameDisplayMonster017`;
 ...
 */
@ccclass
export default class GameDisplayMonster017 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMapForAtk = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayMonster017, GamePlayViewState, number>>;
    /**
     * 获取对应的节点类型
     * @param res 
     * @returns 
    ...
    static GetNodeTypeForAtk (cfg: CfgGameElement) {
        ...
        if (!GameDisplayMonster017._nodeTypeCacheMapForAtk.has(cfg.id)) {
            GameDisplayMonster017._nodeTypeCacheMapForAtk.set(
                cfg.id,
                UINodeType.Pop<GameDisplayMonster017, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayMonster017),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayMonster017, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
