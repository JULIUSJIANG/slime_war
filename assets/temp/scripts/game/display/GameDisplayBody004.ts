...
const APP = `GameDisplayBody004`;

const MAT_PROPERTY_ENABLED_TIME = `enabledTime`;

/**
 * 旋转角锁住，脚底位置对齐，自动同步动画 + 特效时间
 */
@ccclass
export default class GameDisplayBody004 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMap = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayBody004, GamePlayViewState, number>>;
     /**
      * 获取对应的节点类型
      * @param res 
      * @returns 
    ...
    static GetNodeType (cfg: CfgGameElement) {
        ...
        if (!GameDisplayBody004._nodeTypeCacheMap.has(cfg.id)) {
            GameDisplayBody004._nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayBody004, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayBody004),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayBody004, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
