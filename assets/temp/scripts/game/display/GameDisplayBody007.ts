...
const APP = `GameDisplayBody007`;

const MAT_PROPERTY_ENABLED_TIME = `enabledTime`;

/**
 * 旋转角锁住，脚底位置对齐，自动同步动画，特效时间 + 3011 专用
 */
@ccclass
export default class GameDisplayBody007 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMap = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayBody007, GamePlayViewState, number>>;
     /**
      * 获取对应的节点类型
      * @param res 
      * @returns 
    ...
    static GetNodeType (cfg: CfgGameElement) {
        ...
        if (!GameDisplayBody007._nodeTypeCacheMap.has(cfg.id)) {
            GameDisplayBody007._nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayBody007, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayBody007),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayBody007, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
