 ...
 */
@ccclass
export default class GameDisplayBody002 extends UIViewComponent {
    /** 
     * 列表 - 需要时间的节点
    */
     ...
     * 节点缓存
     */
    static _nodeTypeCacheMap = UtilObjPool.Pop(UtilObjPool.typeMap, APP) as Map<number, UINodeType<GameDisplayBody002, GamePlayViewState, number>>;
     /**
      * 获取对应的节点类型
      * @param res 
      * @returns 
    ...
    static GetNodeType (cfg: CfgGameElement) {
        ...
        if (!GameDisplayBody002._nodeTypeCacheMap.has(cfg.id)) {
            GameDisplayBody002._nodeTypeCacheMap.set(
                cfg.id,
                UINodeType.Pop<GameDisplayBody002, GamePlayViewState, number>(
                    APP,
                    {
                        prefabPath: res,
                        componentGetter: node => node.getComponent(GameDisplayBody002),
                        listModuleStyle: [
                            UINodeType.ModuleStyle.Pop<GameDisplayBody002, GamePlayViewState, number>(
                                APP,
                                {
                                    listRefModule: [
