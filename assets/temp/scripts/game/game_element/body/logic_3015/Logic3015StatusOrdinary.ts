...
const APP = `Logic3015StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3015StatusOrdinary extends Logic3015Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3015StatusOrdinary>({
        instantiate: () => {
            return new Logic3015StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3015) {
        let val = UtilObjPool.Pop(Logic3015StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3015Status.ANIM_ORDINARY;
