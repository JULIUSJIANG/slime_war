...
const APP = `Logic3015StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3015StatusCD extends Logic3015Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3015StatusCD>({
        instantiate: () => {
            return new Logic3015StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3015) {
        let val = UtilObjPool.Pop(Logic3015StatusCD._t, apply);
