...
import Logic3015Args from "./Logic3015Args";
import Logic3015Status from "./Logic3015Status";
import Logic3015StatusCD from "./Logic3015StatusCD";
import Logic3015StatusOrdinary from "./Logic3015StatusOrdinary";
...
import Logic3015StatusPower from "./Logic3015StatusPower";
import Logic3015StatusJumped from "./Logic3015StatusJumped";
 ...
 * 经典模板，寻敌、跃击 + 受伤后释放孢子
 */
export default class Logic3015 extends GameElementBodyBehaviour {
    /**
     * 重力缩放
     */
     ...
     * 参数
     */
    public args: Logic3015Args;

    constructor () {
        super();
        this.statusOrdinary = Logic3015StatusOrdinary.Pop(APP, this);
        this.statusPower = Logic3015StatusPower.Pop(APP, this);
        this.statusJumped = Logic3015StatusJumped.Pop(APP, this);
        this.statusCD = Logic3015StatusCD.Pop(APP, this);
    }
 
    private static _t = new UtilObjPoolType<Logic3015>({
        instantiate: () => {
            return new Logic3015();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3015Args) {
        let val = UtilObjPool.Pop(Logic3015._t, apply);
        val.relBody = ge;
        val.args = args;
     ...
     * 当前状态
     */
    public currStatus: Logic3015Status;

    /**
     * 状态 - 常态
     */
    public statusOrdinary: Logic3015StatusOrdinary;
    /**
     * 状态 - 蓄力
     */
    public statusPower: Logic3015StatusPower;
    /**
     * 状态 - 跳跃
     */
    public statusJumped: Logic3015StatusJumped;
    /**
     * 状态 - 已受伤
     */
    public statusCD: Logic3015StatusCD;

    /**
     * 进入某个状态
     * @param status 
     */
    public EnterStatus (status: Logic3015Status) {
    ...
    public OnInit(): void {
        this.relBody.commonCD = GameCD.Pop (APP, this.args.cdSkill);
        ...
        mainBodyDef.gravityScale = Logic3015.GRAVITY_SCALE;
    ...
    public OnDmg(ctx: GameElementBodyCtxDmg): void {
        ...
        if (this.relBody.commonCD.currStatus.OnTryCall()) {
            let angelDistance = Math.PI / 3 / this.args.listSplit.length;
            let speedLen = - this.relBody.relState.b2Gravity * Logic3015.GRAVITY_SCALE / jiang.mgrUI._sizePerPixel / 1000 / 2;
            for (let i = 0; i < this.args.listSplit.length; i++) {
                let angle = Math.PI * 2 / 3 - (i * angelDistance) * (1 + utilMath.RandomLowerToUpper() * this.args.random);
                let eleBody = GameElementBody.PopForCall (
    ...
    
                    {
                        ...
                        vecX: Math.cos (angle) * speedLen * (1 + utilMath.RandomLowerToUpper() * this.args.random),
    ...
    protected Logic3015OnDestory () {

    }

    static rs = GameElementBodyBehaviourRS.Pop<Logic3015Args>(
        APP,
        {
            logicCode: 3015,
            cfgToArgs: (cfg) => {
                return {
                    ...
                    msUp: Number.parseFloat(cfg.prop_12),
                    cdSkill: Number.parseFloat(cfg.prop_13)
                };
            },
            init: (ge, t) => {
                ge.commonBehaviour = Logic3015.Pop(APP, ge, t);
            },
