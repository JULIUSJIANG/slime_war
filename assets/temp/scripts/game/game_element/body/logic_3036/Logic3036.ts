...
import Logic3036Args from "./Logic3036Args";
 ...
 * 箭头 + 穿透定制
 */
class Logic3036 extends GameElementBodyBehaviour {
    /**
     * 参数
     */
    public args: Logic3036Args;

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3036>({
        instantiate: () => {
            return new Logic3036();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3036Args) {
        let val = UtilObjPool.Pop(Logic3036._t, apply);
        val.relBody = ge;
        val.args = args;
        return val;
    }
...
namespace Logic3036 {
    export const rs = GameElementBodyBehaviourRS.Pop<Logic3036Args>(
        APP,
        {
            logicCode: 3036,
            cfgToArgs: (cfg) => {
            ...
            init: (ge, t) => {
                ge.commonBehaviour = Logic3036.Pop(APP, ge, t);
            },
