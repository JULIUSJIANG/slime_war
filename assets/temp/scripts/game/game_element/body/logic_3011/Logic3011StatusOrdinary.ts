...
const APP = `Logic3011StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3011StatusOrdinary extends Logic3011Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3011StatusOrdinary>({
        instantiate: () => {
            return new Logic3011StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3011) {
        let val = UtilObjPool.Pop(Logic3011StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3011Status.ANIM_ORDINARY;
