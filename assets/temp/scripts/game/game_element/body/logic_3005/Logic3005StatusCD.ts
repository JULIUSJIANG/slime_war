...
const APP = `Logic3005StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3005StatusCD extends Logic3005Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3005StatusCD>({
        instantiate: () => {
            return new Logic3005StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3005) {
        let val = UtilObjPool.Pop(Logic3005StatusCD._t, apply);
