...
const APP = `Logic3006StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3006StatusOrdinary extends Logic3006Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3006StatusOrdinary>({
        instantiate: () => {
            return new Logic3006StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3006) {
        let val = UtilObjPool.Pop(Logic3006StatusOrdinary._t, apply);
