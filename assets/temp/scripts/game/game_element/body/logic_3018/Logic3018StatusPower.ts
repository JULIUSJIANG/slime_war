...
const APP = `Logic3018StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3018StatusPower extends Logic3018Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3018StatusPower>({
        instantiate: () => {
            return new Logic3018StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3018) {
        let val = UtilObjPool.Pop(Logic3018StatusPower._t, apply);
    ...
    public OnEnter(): void {
        ...
        this.relMachine.relBody.commonAnim = Logic3018Status.ANIM_POWER;
