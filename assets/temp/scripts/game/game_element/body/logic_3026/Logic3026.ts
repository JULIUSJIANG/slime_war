...
import Logic3026Args from "./Logic3026Args";
import Logic3026Status from "./Logic3026Status";
import Logic3026StatusCD from "./Logic3026StatusCD";
import Logic3026StatusOrdinary from "./Logic3026StatusOrdinary";
...
import Logic3026StatusPower from "./Logic3026StatusPower";
import Logic3026StatusJumped from "./Logic3026StatusJumped";
...
import GameDisplayBody015 from "../../../display/GameDisplayBody015";
 ...
 * 经典模板，寻敌、跃击 + 生命越少速度越快
 */
export default class Logic3026 extends GameElementBodyBehaviour {
    /**
     * 重力缩放
     */
     ...
     * 参数
     */
    public args: Logic3026Args;

    constructor () {
        super();
        this.statusOrdinary = Logic3026StatusOrdinary.Pop(APP, this);
        this.statusPower = Logic3026StatusPower.Pop(APP, this);
        this.statusJumped = Logic3026StatusJumped.Pop(APP, this);
        this.statusCD = Logic3026StatusCD.Pop(APP, this);
    }
 
    private static _t = new UtilObjPoolType<Logic3026>({
        instantiate: () => {
            return new Logic3026();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3026Args) {
        let val = UtilObjPool.Pop(Logic3026._t, apply);
        val.relBody = ge;
        val.args = args;
     ...
     * 当前状态
     */
    public currStatus: Logic3026Status;

    /**
     * 状态 - 常态
     */
    public statusOrdinary: Logic3026StatusOrdinary;
    /**
     * 状态 - 蓄力
     */
    public statusPower: Logic3026StatusPower;
    /**
     * 状态 - 跳跃
     */
    public statusJumped: Logic3026StatusJumped;
    /**
     * 状态 - 已受伤
     */
    public statusCD: Logic3026StatusCD;

    /**
     * 进入某个状态
     * @param status 
     */
    public EnterStatus (status: Logic3026Status) {
    ...
    public OnInit(): void {
        ...
        mainBodyDef.gravityScale = Logic3026.GRAVITY_SCALE;
    ...
    public OnDmg(ctx: GameElementBodyCtxDmg): void {
        ...
        this.relBody.commonTimeScaleSelf = 2 - this.relBody.commonHpCurrent / this.relBody.commonHpMax;
    ...
    protected Logic3026OnDestory () {

    }

    static rs = GameElementBodyBehaviourRS.Pop<Logic3026Args>(
        APP,
        {
            logicCode: 3026,
            cfgToArgs: (cfg) => {
            ...
            init: (ge, t) => {
                ge.commonBehaviour = Logic3026.Pop(APP, ge, t);
            },
            instDisplay: (state, ge, displayList) => {
                displayList.push(GameDisplayBody015.GetNodeType(ge.commonArgsCfg).CreateNode(
                    state,
                    ge.id,
                    ge.id
                ));
