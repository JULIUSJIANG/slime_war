...
const APP = `Logic3026StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3026StatusOrdinary extends Logic3026Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3026StatusOrdinary>({
        instantiate: () => {
            return new Logic3026StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3026) {
        let val = UtilObjPool.Pop(Logic3026StatusOrdinary._t, apply);
        val.relMachine = machine;
        return val;
    }
    
    public OnConcatA(cont: b2Contact<b2Shape, b2Shape>): void {
        cont.SetTangentSpeed(this.GetSpeed() * jiang.mgrUI._sizePerPixel * 1000 * this.relMachine.relBody.commonTimeScaleSelf);
    }
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3026Status.ANIM_ORDINARY;
