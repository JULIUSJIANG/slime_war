...
const APP = `Logic3026StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3026StatusCD extends Logic3026Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3026StatusCD>({
        instantiate: () => {
            return new Logic3026StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3026) {
        let val = UtilObjPool.Pop(Logic3026StatusCD._t, apply);
