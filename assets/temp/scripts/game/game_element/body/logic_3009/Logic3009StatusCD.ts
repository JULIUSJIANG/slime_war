...
const APP = `Logic3009StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3009StatusCD extends Logic3009Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3009StatusCD>({
        instantiate: () => {
            return new Logic3009StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3009) {
        let val = UtilObjPool.Pop(Logic3009StatusCD._t, apply);
