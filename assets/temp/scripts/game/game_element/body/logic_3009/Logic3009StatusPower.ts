...
const APP = `Logic3009StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3009StatusPower extends Logic3009Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3009StatusPower>({
        instantiate: () => {
            return new Logic3009StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3009) {
        let val = UtilObjPool.Pop(Logic3009StatusPower._t, apply);
    ...
    public OnEnter(): void {
        ...
        this.relMachine.relBody.commonAnim = Logic3009Status.ANIM_POWER;
