...
const APP = `Logic3007StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3007StatusCD extends Logic3007Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3007StatusCD>({
        instantiate: () => {
            return new Logic3007StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3007) {
        let val = UtilObjPool.Pop(Logic3007StatusCD._t, apply);
