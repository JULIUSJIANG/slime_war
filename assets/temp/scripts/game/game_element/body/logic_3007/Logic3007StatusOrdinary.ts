...
const APP = `Logic3007StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3007StatusOrdinary extends Logic3007Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3007StatusOrdinary>({
        instantiate: () => {
            return new Logic3007StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3007) {
        let val = UtilObjPool.Pop(Logic3007StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3007Status.ANIM_ORDINARY;
