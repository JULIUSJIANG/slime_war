...
const APP = `Logic3007StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3007StatusPower extends Logic3007Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3007StatusPower>({
        instantiate: () => {
            return new Logic3007StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3007) {
        let val = UtilObjPool.Pop(Logic3007StatusPower._t, apply);
    ...
    public OnEnter(): void {
        ...
        this.relMachine.relBody.commonAnim = Logic3007Status.ANIM_POWER;
