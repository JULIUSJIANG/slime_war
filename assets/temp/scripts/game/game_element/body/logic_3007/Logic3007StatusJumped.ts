...
const APP = `Logic3007StatusJumped`;

/**
 * 状态机 - 状态 - 已跃起
 */
export default class Logic3007StatusJumped extends Logic3007Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3007StatusJumped>({
        instantiate: () => {
            return new Logic3007StatusJumped();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3007) {
        let val = UtilObjPool.Pop(Logic3007StatusJumped._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3007Status.ANIM_JUMPED;
        ...
        vec.y = - this.relMachine.relBody.relState.b2Gravity * this.relMachine.args.atkTime / 1000 / 2 * Logic3007.GRAVITY_SCALE;
