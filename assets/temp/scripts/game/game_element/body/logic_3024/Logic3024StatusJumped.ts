...
const APP = `Logic3024StatusJumped`;

/**
 * 状态机 - 状态 - 已跃起
 */
export default class Logic3024StatusJumped extends Logic3024Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3024StatusJumped>({
        instantiate: () => {
            return new Logic3024StatusJumped();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3024) {
        let val = UtilObjPool.Pop(Logic3024StatusJumped._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3024Status.ANIM_JUMPED;
        ...
        vec.y = - this.relMachine.relBody.relState.b2Gravity * this.relMachine.args.atkTime / 1000 / 2 * Logic3024.GRAVITY_SCALE;
