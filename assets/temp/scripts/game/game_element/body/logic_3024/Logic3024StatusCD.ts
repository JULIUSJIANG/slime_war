...
const APP = `Logic3024StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3024StatusCD extends Logic3024Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3024StatusCD>({
        instantiate: () => {
            return new Logic3024StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3024) {
        let val = UtilObjPool.Pop(Logic3024StatusCD._t, apply);
