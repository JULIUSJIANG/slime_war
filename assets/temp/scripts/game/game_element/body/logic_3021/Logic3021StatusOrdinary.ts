...
const APP = `Logic3021StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3021StatusOrdinary extends Logic3021Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3021StatusOrdinary>({
        instantiate: () => {
            return new Logic3021StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3021) {
        let val = UtilObjPool.Pop(Logic3021StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3021Status.ANIM_ORDINARY;
