...
const APP = `Logic3017StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3017StatusPower extends Logic3017Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3017StatusPower>({
        instantiate: () => {
            return new Logic3017StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3017) {
        let val = UtilObjPool.Pop(Logic3017StatusPower._t, apply);
    ...
    public OnEnter(): void {
        ...
        this.relMachine.relBody.commonAnim = Logic3017Status.ANIM_POWER;
