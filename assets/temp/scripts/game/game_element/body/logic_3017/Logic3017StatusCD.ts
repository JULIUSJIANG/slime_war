...
const APP = `Logic3017StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3017StatusCD extends Logic3017Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3017StatusCD>({
        instantiate: () => {
            return new Logic3017StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3017) {
        let val = UtilObjPool.Pop(Logic3017StatusCD._t, apply);
