...
const APP = `Logic3004StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3004StatusCD extends Logic3004Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3004StatusCD>({
        instantiate: () => {
            return new Logic3004StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3004) {
        let val = UtilObjPool.Pop(Logic3004StatusCD._t, apply);
