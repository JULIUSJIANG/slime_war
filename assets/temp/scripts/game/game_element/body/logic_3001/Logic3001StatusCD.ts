...
const APP = `Logic3001StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3001StatusCD extends Logic3001Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3001StatusCD>({
        instantiate: () => {
            return new Logic3001StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3001) {
        let val = UtilObjPool.Pop(Logic3001StatusCD._t, apply);
