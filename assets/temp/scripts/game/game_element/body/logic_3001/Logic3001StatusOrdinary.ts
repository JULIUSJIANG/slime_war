...
const APP = `Logic3001StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3001StatusOrdinary extends Logic3001Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3001StatusOrdinary>({
        instantiate: () => {
            return new Logic3001StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3001) {
        let val = UtilObjPool.Pop(Logic3001StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3001Status.ANIM_ORDINARY;
