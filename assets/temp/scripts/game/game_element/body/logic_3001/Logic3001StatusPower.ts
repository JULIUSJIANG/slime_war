...
const APP = `Logic3001StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3001StatusPower extends Logic3001Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3001StatusPower>({
        instantiate: () => {
            return new Logic3001StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3001) {
        let val = UtilObjPool.Pop(Logic3001StatusPower._t, apply);
    ...
    public OnEnter(): void {
        GuideMgr.inst.currStatus.P1U6OnBodyWarning ();
        ...
        this.relMachine.relBody.commonAnim = Logic3001Status.ANIM_POWER;
