...
const APP = `Logic3034StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3034StatusOrdinary extends Logic3034Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3034StatusOrdinary>({
        instantiate: () => {
            return new Logic3034StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3034) {
        let val = UtilObjPool.Pop(Logic3034StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3034Status.ANIM_ORDINARY;
