...
const APP = `Logic3034StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3034StatusCD extends Logic3034Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3034StatusCD>({
        instantiate: () => {
            return new Logic3034StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3034) {
        let val = UtilObjPool.Pop(Logic3034StatusCD._t, apply);
