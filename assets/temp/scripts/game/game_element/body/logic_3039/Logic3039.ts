...
import Logic3039Args from "./Logic3039Args";
 ...
 * 箭头 + 非坠落不伤害
 */
class Logic3039 extends GameElementBodyBehaviour {
    /**
     * 参数
     */
    public args: Logic3039Args;
    ...
    private static _t = new UtilObjPoolType<Logic3039>({
        instantiate: () => {
            return new Logic3039();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3039Args) {
        let val = UtilObjPool.Pop(Logic3039._t, apply);
        val.relBody = ge;
        val.args = args;
        return val;
    }
...
namespace Logic3039 {
    export const rs = GameElementBodyBehaviourRS.Pop<Logic3039Args>(
        APP,
        {
            logicCode: 3039,
            cfgToArgs: (cfg) => {
            ...
            init: (ge, t) => {
                ge.commonBehaviour = Logic3039.Pop(APP, ge, t);
            },
