...
import Logic3028Args from "./Logic3028Args";
 ...
 * 孢子射弹
 */
class Logic3028 extends GameElementBodyBehaviour {
    /**
     * 参数
     */
    public args: Logic3028Args;
    ...
    private static _t = new UtilObjPoolType<Logic3028>({
        instantiate: () => {
            return new Logic3028();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3028Args) {
        let val = UtilObjPool.Pop(Logic3028._t, apply);
        val.relBody = ge;
        val.args = args;
        return val;
    }
    ...
    public OnTimeStep(passedMS: number): void {
        if (!this._isFloat && this.relBody.commonBody.b2Body.GetLinearVelocity().y < 0) {
            ...
            vec.x = this.relBody.relState.player.commonBehaviour.b2GetSpeedX() * 0.5 + this.relBody.commonBody.b2Body.GetLinearVelocity().x;
...
namespace Logic3028 {
    export const rs = GameElementBodyBehaviourRS.Pop<Logic3028Args>(
        APP,
        {
            logicCode: 3028,
            cfgToArgs: (cfg) => {
            ...
            init: (ge, t) => {
                ge.commonBehaviour = Logic3028.Pop(APP, ge, t);
            },
