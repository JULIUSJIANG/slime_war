...
import Logic3038Args from "./Logic3038Args";
 ...
 * 箭头 + 坠落分裂
 */
class Logic3038 extends GameElementBodyBehaviour {
    /**
     * 参数
     */
    public args: Logic3038Args;
    ...
    private static _t = new UtilObjPoolType<Logic3038>({
        instantiate: () => {
            return new Logic3038();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, ge: GameElementBody, args: Logic3038Args) {
        let val = UtilObjPool.Pop(Logic3038._t, apply);
        val.relBody = ge;
        val.args = args;
        return val;
    }
    ...
    public OnDestory(): void {
        let angelDistance = Math.PI / 3 / (this.args.listSplit.length - 1);
        let speedLen = - this.relBody.relState.b2Gravity / jiang.mgrUI._sizePerPixel / 1000 / 4;
...
namespace Logic3038 {
    export const rs = GameElementBodyBehaviourRS.Pop<Logic3038Args>(
        APP,
        {
            logicCode: 3038,
            cfgToArgs: (cfg) => {
                return {
                    ...
                    listSplit: utilString.ParseStrToListNum (cfg.prop_3)
                };
            },
            init: (ge, t) => {
                ge.commonBehaviour = Logic3038.Pop(APP, ge, t);
            },
