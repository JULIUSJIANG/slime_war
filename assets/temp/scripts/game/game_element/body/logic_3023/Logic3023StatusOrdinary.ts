...
const APP = `Logic3023StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3023StatusOrdinary extends Logic3023Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3023StatusOrdinary>({
        instantiate: () => {
            return new Logic3023StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3023) {
        let val = UtilObjPool.Pop(Logic3023StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3023Status.ANIM_ORDINARY;
