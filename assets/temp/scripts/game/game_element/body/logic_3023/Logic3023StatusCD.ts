...
const APP = `Logic3023StatusCD`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3023StatusCD extends Logic3023Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3023StatusCD>({
        instantiate: () => {
            return new Logic3023StatusCD();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3023) {
        let val = UtilObjPool.Pop(Logic3023StatusCD._t, apply);
