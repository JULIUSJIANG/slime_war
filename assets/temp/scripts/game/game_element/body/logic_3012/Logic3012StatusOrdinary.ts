...
const APP = `Logic3012StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3012StatusOrdinary extends Logic3012Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3012StatusOrdinary>({
        instantiate: () => {
            return new Logic3012StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3012) {
        let val = UtilObjPool.Pop(Logic3012StatusOrdinary._t, apply);
