...
const APP = `Logic3008StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3008StatusOrdinary extends Logic3008Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3008StatusOrdinary>({
        instantiate: () => {
            return new Logic3008StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3008) {
        let val = UtilObjPool.Pop(Logic3008StatusOrdinary._t, apply);
    ...
    public OnEnter(): void {
        this.relMachine.relBody.commonAnim = Logic3008Status.ANIM_ORDINARY;
