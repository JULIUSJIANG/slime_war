...
const APP = `Logic3008StatusPower`;

/**
 * 状态机 - 状态 - 已蓄力
 */
export default class Logic3008StatusPower extends Logic3008Status {

    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3008StatusPower>({
        instantiate: () => {
            return new Logic3008StatusPower();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3008) {
        let val = UtilObjPool.Pop(Logic3008StatusPower._t, apply);
    ...
    public OnEnter(): void {
        ...
        this.relMachine.relBody.commonAnim = Logic3008Status.ANIM_POWER;
