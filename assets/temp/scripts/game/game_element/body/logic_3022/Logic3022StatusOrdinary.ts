...
const APP = `Logic3022StatusOrdinary`;

/**
 * 状态机 - 状态 - 已受伤
 */
export default class Logic3022StatusOrdinary extends Logic3022Status{
    
    private constructor () {
        super();
    }

    private static _t = new UtilObjPoolType<Logic3022StatusOrdinary>({
        instantiate: () => {
            return new Logic3022StatusOrdinary();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, machine: Logic3022) {
        let val = UtilObjPool.Pop(Logic3022StatusOrdinary._t, apply);
