...
const APP = `GEEI_2006_status_ready`;

/**
 * 就绪
 */
export default class GEEI_2006_status_ready extends GEEI_2006_status {

    private constructor () {
        super ();
    }

    private static _t = new UtilObjPoolType<GEEI_2006_status_ready>({
        instantiate: () => {
            return new GEEI_2006_status_ready ();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, rel: GEEI_2006) {
        let val = UtilObjPool.Pop (GEEI_2006_status_ready._t, apply);
        val.rel = rel;
        return val;
    }
