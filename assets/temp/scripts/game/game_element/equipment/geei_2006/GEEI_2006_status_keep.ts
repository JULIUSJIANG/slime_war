...
const APP = `GEEI_2006_status_keep`;

/**
 * 蓄力
 */
export default class GEEI_2006_status_keep extends GEEI_2006_status {

    private constructor () {
        super ();
    }

    private static _t = new UtilObjPoolType<GEEI_2006_status_keep>({
        instantiate: () => {
            return new GEEI_2006_status_keep ();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, rel: GEEI_2006) {
        let val = UtilObjPool.Pop (GEEI_2006_status_keep._t, apply);
        val.rel = rel;
        return val;
    }
