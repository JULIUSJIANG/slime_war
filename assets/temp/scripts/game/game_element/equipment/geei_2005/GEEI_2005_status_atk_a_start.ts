...
const APP = `GEEI_2005_status_a_start`;

/**
 * 前摇
 */
export default class GEEI_2005_status_a_start extends GEEI_2005_status {

    private constructor () {
        super ();
    }

    private static _t = new UtilObjPoolType<GEEI_2005_status_a_start>({
        instantiate: () => {
            return new GEEI_2005_status_a_start ();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, rel: GEEI_2005) {
        let val = UtilObjPool.Pop (GEEI_2005_status_a_start._t, apply);
        val.rel = rel;
        return val;
    }
