...
const APP = `GEEI_2005_status_c_end`;

/**
 * 后摇
 */
export default class GEEI_2005_status_c_end extends GEEI_2005_status {

    private constructor () {
        super ();
    }

    private static _t = new UtilObjPoolType<GEEI_2005_status_c_end>({
        instantiate: () => {
            return new GEEI_2005_status_c_end ();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, rel: GEEI_2005) {
        let val = UtilObjPool.Pop (GEEI_2005_status_c_end._t, apply);
        val.rel = rel;
        return val;
    }
