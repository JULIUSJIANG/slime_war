...
import { GEEI_2004_args } from "./GEEI_2004_args";
import GEEI_2004_status from "./GEEI_2004_status";
import GEEI_2004_status_a_start from "./GEEI_2004_status_atk_a_start";
import GEEI_2004_status_b_ing from "./GEEI_2004_status_atk_b_ing";
import GEEI_2004_status_c_end from "./GEEI_2004_status_atk_c_end";
import GEEI_2004_status_cd from "./GEEI_2004_status_cd";
import GEEI_2004_status_keep from "./GEEI_2004_status_keep";
import GEEI_2004_status_ready from "./GEEI_2004_status_ready";
 ...
 * 类似 cf 枪支的武器模板
 */
class GEEI_2004 extends EquipmentInst {

    private static _t = new UtilObjPoolType<GEEI_2004>({
        instantiate: () => {
            return new GEEI_2004();
        },
        onPop: (t) => {
    ...
    {
        let val = UtilObjPool.Pop (GEEI_2004._t, apply);
        val.args = args;
        return val;
    }

    OnInit(): void {
        this.statusAtkAStart = GEEI_2004_status_a_start.Pop (APP, this);
        this.statusAtkBIng = GEEI_2004_status_b_ing.Pop (APP, this);
        this.statusAtkCEnd = GEEI_2004_status_c_end.Pop (APP, this);
        this.statusCD = GEEI_2004_status_cd.Pop (APP, this);
        this.statusKeep = GEEI_2004_status_keep.Pop (APP, this);
        this.statusReady = GEEI_2004_status_ready.Pop (APP, this);
     ...
     * 状态 - 攻击 - 前摇
     */
    statusAtkAStart: GEEI_2004_status_a_start;
    /**
     * 状态 - 攻击 - 进行时
     */
    statusAtkBIng: GEEI_2004_status_b_ing;
    /**
     * 状态 - 攻击 - 后摇
     */
    statusAtkCEnd: GEEI_2004_status_c_end;
    /**
     * 状态 - 攻击 - 冷却
     */
    statusCD: GEEI_2004_status_cd;
    /**
     * 状态 - 攻击 - 蓄力
     */
    statusKeep: GEEI_2004_status_keep;
    /***
     * 状态 - 攻击 - 就绪
     */
    statusReady: GEEI_2004_status_ready;

    /**
     * 当前状态
     */
    currStatus: GEEI_2004_status;

    EnterStatus (status: GEEI_2004_status) {
    ...
    UpdateAim () {
        ...
        // 垂直方向
        this.vy = ((this.aimPosition.y - this.relEquip.npc.playerEquipPos.y) - 0.5 * this.bulletGravityScale * this.relEquip.npc.relBody.relState.b2Gravity / jiang.mgrUI._sizePerPixel / (1000 ** 2) * this.tTotal ** 2) / this.tTotal;
        // 水平方向
    ...
    Fire () {
        ...
        let ran = Math.random ();
        ran = Math.pow (ran, 2.0);
        ran = Math.random () < 0.5 ? ran : -ran;
        this.angleOffsetFire = this.angleOffset * ran;
...
EquipmentInstRS.Pop<GEEI_2004_args> (
    APP,
    {
        logicCode: 2004,
        ...
        instCreator: (apply: string, args: GEEI_2004_args) => {
            return GEEI_2004.Pop (apply, args);
        },
        instDisplay: (state, npc, equipList) => {
            ...
            switch (equipmentInst.cfgProps.style) {
                ...
                case 1004: {
                    equipList.push(GameDisplayEquipment004.GetNodeTypeByRes(equipmentInst.cfgProps).CreateNode(
                        state,
                        npc.id,
                        -npc.id
                    ));
