...
const APP = `GEEI_2004_status_cd`;

/**
 * 冷却
 */
export default class GEEI_2004_status_cd extends GEEI_2004_status {

    private constructor () {
        super ();
    }

    private static _t = new UtilObjPoolType<GEEI_2004_status_cd>({
        instantiate: () => {
            return new GEEI_2004_status_cd ();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, rel: GEEI_2004) {
        let val = UtilObjPool.Pop (GEEI_2004_status_cd._t, apply);
        val.rel = rel;
        return val;
    }
