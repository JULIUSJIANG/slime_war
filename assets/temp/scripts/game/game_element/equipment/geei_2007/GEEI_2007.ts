...
import GameDisplayEquipment007 from "../../../display/GameDisplayEquipment007";
...
import { GEEI_2007_args } from "./GEEI_2007_args";
import GEEI_2007_status from "./GEEI_2007_status";
import GEEI_2007_status_a_start from "./GEEI_2007_status_atk_a_start";
import GEEI_2007_status_b_ing from "./GEEI_2007_status_atk_b_ing";
import GEEI_2007_status_c_end from "./GEEI_2007_status_atk_c_end";
import GEEI_2007_status_cd from "./GEEI_2007_status_cd";
import GEEI_2007_status_keep from "./GEEI_2007_status_keep";
import GEEI_2007_status_ready from "./GEEI_2007_status_ready";

const APP = `GEEI_2007`;

/**
 * 类似 cf 枪支的武器模板 + 持续输出 + 水晶球定制
 */
class GEEI_2007 extends EquipmentInst {

    private static _t = new UtilObjPoolType<GEEI_2007>({
        instantiate: () => {
            return new GEEI_2007();
        },
        onPop: (t) => {
    ...
    {
        let val = UtilObjPool.Pop (GEEI_2007._t, apply);
        val.args = args;
        return val;
    }

    OnInit(): void {
        this.statusAtkAStart = GEEI_2007_status_a_start.Pop (APP, this);
        this.statusAtkBIng = GEEI_2007_status_b_ing.Pop (APP, this);
        this.statusAtkCEnd = GEEI_2007_status_c_end.Pop (APP, this);
        this.statusCD = GEEI_2007_status_cd.Pop (APP, this);
        this.statusKeep = GEEI_2007_status_keep.Pop (APP, this);
        this.statusReady = GEEI_2007_status_ready.Pop (APP, this);
     ...
     * 状态 - 攻击 - 前摇
     */
    statusAtkAStart: GEEI_2007_status_a_start;
    /**
     * 状态 - 攻击 - 进行时
     */
    statusAtkBIng: GEEI_2007_status_b_ing;
    /**
     * 状态 - 攻击 - 后摇
     */
    statusAtkCEnd: GEEI_2007_status_c_end;
    /**
     * 状态 - 攻击 - 冷却
     */
    statusCD: GEEI_2007_status_cd;
    /**
     * 状态 - 攻击 - 蓄力
     */
    statusKeep: GEEI_2007_status_keep;
    /***
     * 状态 - 攻击 - 就绪
     */
    statusReady: GEEI_2007_status_ready;

    /**
     * 当前状态
     */
    currStatus: GEEI_2007_status;

    EnterStatus (status: GEEI_2007_status) {
     ...
     */
    UpdateAim () {
        this.aimPosition.x = this.relEquip.firePos.x;
        this.aimPosition.y = this.relEquip.firePos.y;
        ...
        let lenCurrent = Math.sqrt ((this.aimPosition.x - this.GetEquipPosX()) ** 2 + (this.aimPosition.y - this.GetEquipPosY()) ** 2);
        // 垂直方向
        this.tTotal = this.args.msGot * lenCurrent / lenMax;
        let gravity = this.bulletGravityScale;
        this.vy = ((this.aimPosition.y - this.GetEquipPosY()) - 0.5 * gravity * this.relEquip.npc.relBody.relState.b2Gravity / jiang.mgrUI._sizePerPixel / (1000 ** 2) * this.tTotal ** 2) / this.tTotal;
        // 水平方向
        this.vx = (this.aimPosition.x - this.GetEquipPosX()) / this.tTotal;
        // 由水平速度、垂直速度，得到抛射角
    ...
    Fire () {
        ...
        let eleBody = GameElementBody.Pop(
            ...
            {
                posX: this.GetEquipPosX() + dx + radius * utilMath.RandomLowerToUpper(),
                posY: this.GetEquipPosY() + dy,
    ...
    GetEquipPosX () {
        let len = this.GetEquipPosY () - this.aimPosition.y;
        return this.aimPosition.x - len;
    }

    GetEquipPosY () {
        return gameCommon.GROUND_Y + (MgrUI.FAT_WIDTH + gameCommon.CAMERA_OFFSET_Y + 50) / gameCommon.SCENE_SCALE;
    }
}

EquipmentInstRS.Pop<GEEI_2007_args> (
    APP,
    {
        logicCode: 2007,
        ...
        instCreator: (apply: string, args: GEEI_2007_args) => {
            return GEEI_2007.Pop (apply, args);
        },
        instDisplay: (state, npc, equipList) => {
            ...
            switch (equipmentInst.cfgProps.style) {
                ...
                case 1007: {
                    equipList.push(GameDisplayEquipment007.GetNodeTypeByRes(equipmentInst.cfgProps).CreateNode(
                        state,
                        npc.id,
                        -npc.id
                    ));
