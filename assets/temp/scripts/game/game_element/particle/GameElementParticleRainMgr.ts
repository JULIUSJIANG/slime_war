...
const APP = `GameElementParticleRainMgr`;
 ...
 * 降雨管理器
 */
export default class GameElementParticleRainMgr {

    private constructor () {}

    private static _t = new UtilObjPoolType<GameElementParticleRainMgr>({
        instantiate: () => {
            return new GameElementParticleRainMgr();
        },
        onPop: (t) => {
    ...
    static Pop (apply: string, gameState: GameState) {
        let val = UtilObjPool.Pop(GameElementParticleRainMgr._t, apply);
    ...
    private Drop () {
        ...
        // 下落时间
        let dropTime = dropY / this.speedY;
        // 坠落水平偏移
        let dropX = dropTime * (this.speedX - playerSpeedX);
        ...
        let rain = GameElementParticleRain.Pop(
            APP,
    ...
    WarmUp () {
        ...
        let dropY = (gameCommon.GROUND_Y - MgrUI.FAT_HEIGHT) / gameCommon.SCENE_SCALE;
        // 坠落耗时
        let flowMS = dropY / this.speedY - 1;
        // 自我进行时间推进
