...
import GameSuccessedViewState from "./GameSuccessedViewState";
...
const APP = `GameSuccessedView`;

/**
 * 胜利界面
 */
@ccclass
export default class GameSuccessedView extends UIViewComponent {
    /**
     * 按钮 - 返回主页
     */
     ...
     */
    @property(ComponentNodeEventer)
    btnNext: ComponentNodeEventer = null;
    ...
    static nodeType = UINodeType.Pop<GameSuccessedView, GameSuccessedViewState, number>(
        APP,
        {
            prefabPath: `${gameCommon.SUB_MAIN}/prefab_ui/game_successed_view`,
            componentGetter: node => node.getComponent(GameSuccessedView),
            listModuleStyle: [
                UINodeType.ModuleStyle.Pop<GameSuccessedView, GameSuccessedViewState, number>(
                    APP,
                    {
                        listRefModule: [
                        ...
                        propsFilter: (com, state, props) => {
                            ...
                            com.btnNext.evterTouchStart.On(() => {
                                VoiceOggViewState.inst.VoiceSet(gameCommon.BTN_VOICE);
                            });
                            com.btnNext.evterTouchEnd.On(() => {
                                RewardViewState.GotRewardOrdinary (gameCommon.TITLE_REWARD, state.gameState.listRewardIdCfg)
                                    .then (() => {
            ...
            childrenCreator: [
                UINodeType.ChildrenGeneration.Pop<GameSuccessedView, GameSuccessedViewState, number>(
                    APP,
                    {
